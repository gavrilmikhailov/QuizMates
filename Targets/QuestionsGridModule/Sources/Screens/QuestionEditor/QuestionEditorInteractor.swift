//
//  QuestionEditorInteractor.swift
//  QuizMates
//
...
@MainActor
protocol QuestionEditorInteractorProtocol {
...
@MainActor
final class QuestionEditorInteractor: QuestionEditorInteractorProtocol {

    // MARK: - Internal properties

    weak var view: QuestionEditorViewControllerProtocol?

    // MARK: - Private properties

    private let databaseService: DatabaseServiceProtocol
    private let topic: TopicDTO
    private let mode: QuestionEditorMode
    private var medias: [MediaDTO] = []
    private var mediaDrafts: [MediaDraft] = []
    ...
    // MARK: - QuestionEditorInteractorProtocol
    ...
    func loadQuestionContent(isInitial: Bool) {
        ...
        case .editExistingQuestion(let dto):
            Task {
                do {
                    medias = try await databaseService.fetch(ids: dto.medias) { model in
            ...
            for item in items {
                do {
                    ...
                    if isVideo {
                        ...
                        guard let thumbnailData = QuestionsGridModuleAsset.videoPlaceholder.image.jpegData(compressionQuality: 1) else {
                            continue
                        }
    ...
    func addMediaItems(audios: [URL]) {
        Task {
            for url in audios {
                ...
                guard let thumbnailData = QuestionsGridModuleAsset.audioPlaceholder.image.jpegData(compressionQuality: 1) else {
                    continue
                }
                do {
                    let draft = MediaDraft(
